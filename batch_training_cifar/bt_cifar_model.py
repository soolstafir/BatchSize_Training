...
def model():
    ...
    with tf.name_scope('data'):
        INPUT_DIMENTION = IMAGE_SIZE * IMAGE_SIZE * IMAGE_CHANNELS
        x = tf.placeholder(
                tf.float32, shape=[None, INPUT_DIMENTION], name='Input')
        y = tf.placeholder(
                tf.float32, shape=[None, NUM_CLASSES], name='Output')
        x_image = tf.reshape(
                x, [-1, IMAGE_SIZE, IMAGE_SIZE, IMAGE_CHANNELS], name='images')
    ...
    norm1 = tf.nn.lrn(
            conv1, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75, name='norm1')
    ...
    norm3 = tf.nn.lrn(
            conv5, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75, name='norm3')
    ...
    tf.summary.scalar(
            'Fully connected layers/fc1', tf.nn.zero_fraction(local3))
    ...
    tf.summary.scalar(
            'Fully connected layers/fc2', tf.nn.zero_fraction(local4))
